...
def get_tasks_by_id(id:int, db:Session):
...
def get_tasks_by_tags(tags:TaskProgress, db:Session):
    task = db.query(Tasks).filter(Tasks.tags == tags.value).all()
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Task with tag {tags} not found"
        )
    return task
...
def update_task(id:int, task_data:schema.TaskCreate, db:Session):
    ...
    task.tags = task_data.tags.value
